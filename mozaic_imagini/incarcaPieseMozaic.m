...
if ~isempty(params.numeDirector)
    filelist = dir([params.numeDirector '/*.' params.tipImagine]);

    imgNm = [params.numeDirector '/' filelist(1).name];
    img = imread(imgNm);

    pieseMozaic = uint8(zeros([size(img) length(filelist)]));

    for idxImg = 1:length(filelist)
        imgName = [params.numeDirector '/' filelist(idxImg).name];
        nextImg = imread(imgName);

        pieseMozaic(:,:,:,idxImg) = nextImg;
    end

else
    pieseMozaic = params.pieseMozaic;
end
