...
idxEx = 1;
% redimensioneaza imaginile si apoi obtine exemple negative pentru fiecare
% dimensiune
...
function descriptoriExempleNegative = obtineDescriptoriExempleNegative2(parametri)
% descriptoriExempleNegative = matrice MxD, unde:
%   M = numarul de exemple negative de antrenare (NU sunt fete de oameni),
%   M = parametri.numarExempleNegative
%   D = numarul de dimensiuni al descriptorului
...
for idx = 1:numarImagini
    ...
    %completati codul functiei in continuare
    [redimLinii, redimColoane] = obtineDimensiuniRedimensionare(img, 1.1, 0.8);
    
    for i = 1:length(redimLinii)
    
        imgRedim = imresize(img, [redimLinii(i), redimColoane(i)]);
        
        inaltime = size(imgRedim, 1);
        latime = size(imgRedim, 2);
        ...
        for j = 1:numarExempleNegative_pe_imagineaCurenta
            patch = imgRedim(x(j):x(j)+parametri.dimensiuneFereastra-1, y(j):y(j)+parametri.dimensiuneFereastra-1);
            descriptor = vl_hog(single(patch), parametri.dimensiuneCelulaHOG);
            descriptoriExempleNegative(idxEx, :) = descriptor(:);
            idxEx = idxEx + 1;
        end
